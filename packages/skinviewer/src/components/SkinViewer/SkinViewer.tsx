...
import { useEffect, useRef, type HTMLAttributes, type Ref, type RefObject } from 'react';
...
export interface SkinViewerProps extends HTMLAttributes<HTMLDivElement> {
  getRef?: RefObject<SkinViewerBase | null>;
...
export function SkinViewer({
  ...
  const viewerRef = useRef<SkinViewerBase>(null);
  const mergedViewerRef = useMergedRef(...[viewerRef, getRef].filter((value) => value !== undefined));
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        onClick={() => {
          console.log(viewerRef.current, viewerRef.current?.camera.position, viewerRef.current?.camera.rotation);
        }}
      />
    </div>
  );
