...
export class SessionService {
  ...
  private _distinctIdCache = this._cache.createAtom<string>('distinctId', SyncAtomType.STRING);
  ...
  private _distinctId: string | null = this._distinctIdCache.get();
  ...
  readonly api = createClient<APIPaths>({
    ...
    fetch: async (request) => {
      if (this._distinctId) {
        request.headers.set('Distinct-Id', this._distinctId);
      }
      ...
      const distinctId = response.headers.get('Distinct-Id');

      if (distinctId) {
        this._distinctId = distinctId;
        this._distinctIdCache.set(distinctId);
      }
