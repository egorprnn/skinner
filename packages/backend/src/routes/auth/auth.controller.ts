...
import { AuthMinecraftSyncException } from './error/auth-minecraft-sync.error';
...
export class AuthController {
  ...
  async microsoft(@Body() authMicrosoftDto: AuthMicrosoftDto, @Res({ passthrough: true }) context: Context) {
    this.posthog.capture({
      distinctId: context.req.distinctId,
      event: 'auth_start',
    });
    ...
    const user = await this.userService
      .findByMicrosoftAuthenticationResultOrCreate(
        microsoftAuthenticationResult as DeepNonNullable<AuthenticationResult>,
      )
      .catch(() => null);

    if (!user) {
      throw new AuthMinecraftSyncException();
    }
    ...
    this.posthog.identify({
      distinctId: user.microsoft_id,
      properties: user,
    });
    this.posthog.alias({
      distinctId: context.req.distinctId,
      alias: user.microsoft_id,
    });
